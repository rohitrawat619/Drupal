...
/**
 * Provides a repository for entity display objects (transform modes).
 */
class EntityTransformRepository implements EntityTransformRepositoryInterface {
  ...
  public function getAllTransformModes() {
    return $this->getAllDisplayModesByEntityType('transform_mode');
  }
  ...
  public function getTransformModes($entity_type_id) {
    return $this->getDisplayModesByEntityType('transform_mode', $entity_type_id);
  }
   ...
   *   The display type to be retrieved. It can be "transform_mode".
   *
   * @return array
  ...
  protected function getAllDisplayModesByEntityType($display_type) {
    if (!isset($this->displayModeInfo[$display_type])) {
      ...
      if ($cache = $this->cacheGet("$key:$langcode")) {
        ...
        $this->cacheSet(
          "$key:$langcode", $this->displayModeInfo[$display_type],
  ...
  public function getTransformModeOptions($entity_type_id) {
    return $this->getDisplayModeOptions('transform_mode', $entity_type_id);
  }
  ...
  public function getTransformModeOptionsByBundle($entity_type_id, $bundle) {
    return $this->getDisplayModeOptionsByBundle('transform_mode', $entity_type_id, $bundle);
  }
  ...
  protected function getDisplayModeOptionsByBundle($display_type, $entity_type_id, $bundle) {
    ...
    $displays = $this->entityTypeManager
      ->getStorage('entity_transform_display')
  ...
  public function getTransformDisplay($entity_type, $bundle, $transform_mode = self::DEFAULT_DISPLAY_MODE) {
    $storage = $this->entityTypeManager->getStorage('entity_transform_display');

    // Try loading the display from configuration; if not found, create a fresh
    // display object. We do not preemptively create new
    // entity_transform_display configuration entries for each existing entity
    // type and bundle whenever a new transform mode becomes available. Instead,
    // configuration entries are only created when a display object is
    // explicitly configured and saved.
    $entity_transform_display = $storage->load($entity_type . '.' . $bundle . '.' . $transform_mode);
    if (!$entity_transform_display) {
      $entity_transform_display = $storage->create([
    ...
    return $entity_transform_display;
  }
